...
const PLATFORM = window.PigEscapePlatform ?? {};
const PLATFORM_STORAGE = PLATFORM.storage ?? {};
const PLATFORM_AUDIO = PLATFORM.audio ?? {};
const PLATFORM_ADS = PLATFORM.ads ?? {};

function getStoredString(key, fallback = null) {
  if (typeof PLATFORM_STORAGE.getString === "function") {
    return PLATFORM_STORAGE.getString(key, fallback);
  }
  return fallback;
}

function setStoredString(key, value) {
  if (typeof PLATFORM_STORAGE.setString === "function") {
    return PLATFORM_STORAGE.setString(key, value);
  }
  return false;
}

function getStoredJSON(key, fallback = null) {
  if (typeof PLATFORM_STORAGE.getJSON === "function") {
    return PLATFORM_STORAGE.getJSON(key, fallback);
  }
  try {
    const raw = getStoredString(key, null);
...
function setStoredJSON(key, value) {
  if (typeof PLATFORM_STORAGE.setJSON === "function") {
    return PLATFORM_STORAGE.setJSON(key, value);
  }
  return false;
}

function createAudioSource(src, options = {}) {
  if (typeof PLATFORM_AUDIO.createSource === "function") {
    return PLATFORM_AUDIO.createSource(src, options);
  }
  return null;
}

async function requestRewardedAd(placement, context = {}) {
  if (typeof PLATFORM_ADS.showRewardedVideo !== "function") {
    return { ok: false, reason: "not_configured" };
  }
  return PLATFORM_ADS.showRewardedVideo({ placement, context });
}
...
const AUTO_LEVEL_ANIMAL_COUNT = 60;
const AUTO_LEVEL_OPEN_COUNT = 3;
const AUTO_LEVEL_STAR_THRESHOLDS = [0, 625, 750];
const AUTO_LEVEL_GENERATOR_ATTEMPTS = 260;
const AUTO_LEVEL_REPAIR_PASSES = 90;

const LEVELS = window.PIG_ESCAPE_LEVELS ?? [];
const EDITOR_LEVELS_STORAGE_KEY = "pigEscapeEditorDeployedLevelsV2Board10x16";
const AUTO_GENERATED_LEVELS_STORAGE_KEY = "pigEscapeAutoGeneratedLevelsV1Board10x16";
...
function getGameLevels() {
  const baseLevels = getBaseGameLevels();
  return [
    ...baseLevels,
    ...loadAutoGeneratedLevels(baseLevels),
  ];
}

function getBaseGameLevels() {
  const levels = getStoredJSON(EDITOR_LEVELS_STORAGE_KEY, null);
...
function loadAutoGeneratedLevels(baseLevels = getBaseGameLevels()) {
  if (isDevToolTestMode()) return [];

  const payload = getStoredJSON(AUTO_GENERATED_LEVELS_STORAGE_KEY, null);
  if (!payload || payload.sourceKey !== getLevelSourceKey(baseLevels)) return [];
  if (!isLevelSetCompatibleWithBoard(payload.levels)) return [];
  return payload.levels;
}

function saveAutoGeneratedLevels(baseLevels, levels) {
  if (isDevToolTestMode()) return;

  setStoredJSON(AUTO_GENERATED_LEVELS_STORAGE_KEY, {
    sourceKey: getLevelSourceKey(baseLevels),
    levels,
  });
}

function getLevelSourceKey(levels) {
  const lastLevel = levels[levels.length - 1] ?? {};
  return [
    levels.length,
    lastLevel.id ?? "",
    lastLevel.animals?.length ?? 0,
    lastLevel.starThresholds?.join("-") ?? "",
  ].join(":");
}

function ensureNextLevelAvailable() {
  const levels = getGameLevels();
  if (state.levelIndex + 1 < levels.length) return true;

  const baseLevels = getBaseGameLevels();
  const generatedLevels = loadAutoGeneratedLevels(baseLevels);
  const nextLevelNumber = baseLevels.length + generatedLevels.length + 1;
  const nextLevel = generateAutoLevel(nextLevelNumber);
  if (!nextLevel) return false;

  saveAutoGeneratedLevels(baseLevels, [...generatedLevels, nextLevel]);
  renderLevelSelect();
...
function generateAutoLevel(levelNumber) {
  const area = getDefaultPlayArea();
  const animals = generateLevelAnimals(
    AUTO_LEVEL_ANIMAL_COUNT,
    area,
    AUTO_LEVEL_OPEN_COUNT,
  );
  ...
  return {
    ...
    playArea: area,
    animals,
    starThresholds: AUTO_LEVEL_STAR_THRESHOLDS,
  };
}

function generateLevelAnimals(targetCount, area, targetOpenCount = null) {
  let best = null;
  for (let attempt = 0; attempt < AUTO_LEVEL_GENERATOR_ATTEMPTS; attempt += 1) {
    ...
    const validation = validateGeneratedLevelAnimals(animals, area);
    const score = getGenerationScore(validation, targetCount, targetOpenCount);
    if (!best || score < best.score) {
      best = { animals: animals.map(stripGeneratedAnimal), score, validation };
    }
    if (isGenerationGood(validation, targetCount, targetOpenCount)) {
      return animals.map(stripGeneratedAnimal);
    }
  }
...
function seedGeneratedAnimals(targetCount, area) {
  ...
  while (animals.length < targetCount && misses < candidates.length * 8) {
    ...
    const animal = {
      ...candidate,
      id: `auto-gen-${animals.length}-${Date.now()}-${Math.round(Math.random() * 100000)}`,
      active: true,
    };
  ...
  for (let y = area.y; y < area.y + area.rows; y += 1) {
    for (let x = area.x; x < area.x + area.cols; x += 1) {
      DIRECTIONS.forEach((dir) => {
        ...
        if (isAnimalOnValidCells(animal, area)) candidates.push({ x, y, dir });
      });
    }
  }
  return candidates;
...
function repairGeneratedAnimals(animals, area, targetOpenCount = null) {
  for (let pass = 0; pass < AUTO_LEVEL_REPAIR_PASSES; pass += 1) {
    const validation = validateGeneratedLevelAnimals(animals, area);
...
function improveGeneratedAnimal(animals, animalId, area, currentValidation, targetOpenCount = null) {
  ...
  for (const option of options) {
    ...
    const score = getGenerationScore(validateGeneratedLevelAnimals(animals, area), animals.length, targetOpenCount);
...
function validateGeneratedLevelAnimals(animals, area) {
  const validation = validateBoardLevelAnimals(animals, area);
...
function canPlaceInList(animal, animals, ignoreId, area) {
  if (!isAnimalOnValidCells(animal, area)) return false;
...
function stripGeneratedAnimal(animal) {
  return {
    x: animal.x,
    y: animal.y,
    dir: animal.dir,
...
function getAudioSource(key) {
  ...
  if (!file) return null;
  const audio = createAudioSource(`${AUDIO_BASE_PATH}${file}`, {
    preload: "none",
    volume: AUDIO_VOLUMES[key] ?? 0.35,
...
function loadAudioEnabled() {
  return getStoredString(AUDIO_ENABLED_STORAGE_KEY, "1") !== "0";
}

function saveAudioEnabled() {
  setStoredString(AUDIO_ENABLED_STORAGE_KEY, state.audioEnabled ? "1" : "0");
}
...
function loadProgress() {
  ...
  return getStoredJSON(PROGRESS_STORAGE_KEY, {});
}

function loadEquippedSkin() {
  const key = getStoredString(EQUIPPED_SKIN_STORAGE_KEY, null);
...
function loadUnlockedCollection() {
  ...
  const saved = getStoredJSON(UNLOCKED_COLLECTION_STORAGE_KEY, null);
...
function saveUnlockedCollection() {
  ...
  setStoredJSON(UNLOCKED_COLLECTION_STORAGE_KEY, [...state.unlockedCollection]);
}
...
function loadEquippedTools() {
  ...
  const saved = getStoredJSON(EQUIPPED_TOOLS_STORAGE_KEY, null);
...
function saveEquippedTools() {
  ...
  setStoredJSON(EQUIPPED_TOOLS_STORAGE_KEY, [...state.equippedTools]);
}
...
function loadEnabledAbilities() {
  ...
  const saved = getStoredJSON(ENABLED_ABILITIES_STORAGE_KEY, null);
...
function saveEnabledAbilities() {
  ...
  setStoredJSON(ENABLED_ABILITIES_STORAGE_KEY, [...state.enabledAbilities]);
}

function saveEquippedSkin(key) {
  if (isDevToolTestMode()) return;

  setStoredString(EQUIPPED_SKIN_STORAGE_KEY, key);
}

function saveProgress() {
  if (isDevToolTestMode()) return;

  setStoredJSON(PROGRESS_STORAGE_KEY, state.bestByLevel);
}
...
function loadFirecrackerPosition() {
  ...
  const position = getStoredJSON(FIRECRACKER_POSITION_KEY, null);
  if (
    !position
...
function saveFirecrackerPosition(position) {
  ...
  setStoredJSON(FIRECRACKER_POSITION_KEY, position);
}
...
nextLevelBtn.addEventListener("click", () => {
  if (state.levelIndex + 1 >= getGameLevels().length && !ensureNextLevelAvailable()) return;
